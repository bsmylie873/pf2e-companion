...
export function listMapPinGroups(gameId: string, mapId: string): Promise<PinGroup[]> {
  return apiFetch<PinGroup[]>(`/games/${gameId}/maps/${mapId}/pin-groups`)
}

export function createMapPinGroup(gameId: string, mapId: string, pinIds: string[]): Promise<PinGroup> {
  return apiFetch<PinGroup>(`/games/${gameId}/maps/${mapId}/pin-groups`, {
    method: 'POST',
